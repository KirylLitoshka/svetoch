...
from sqlalchemy import delete, insert, select, update, literal_column
...
class ListView(BaseView):
    ...
    async def post(self):
        async with self.request.app["db"].begin() as conn:
            post_data = await self.request.json()
            cursor = await conn.execute(self.model.insert().returning(literal_column('*')).values(**post_data))
            result = dict(cursor.fetchone())
            return web.json_response({"success": True, "item": result}, status=201, dumps=pretty_json)
